...
describe("Generate a config utilizing Workspaces", () => {
	...
	const myExecutor = new CircleCI.Executor.DockerExecutor("my-executor", "cimg/base:stable")
	myConfig.addExecutor(myExecutor)

	// Create Jobs
	const jobFlow = new CircleCI.Job("flow", myExecutor, [new CircleCI.Command.Workspace.Persist({root: "workspace", paths: ["echo-output"]})])
	myConfig.addJob(jobFlow)

	const jobDownstream = new CircleCI.Job("downstream", myExecutor, [new CircleCI.Command.Workspace.Attach({at: "/tmp/workspace"})])
