...
import { Executable } from '../Executors/exports/Executable';
 ...
 */
export class Job extends Executable {
  private _steps?: AnyCommand[];
  /**
   ...
   */
  constructor(name: string, executor: AnyExecutor, steps?: AnyCommand[]) {
    super(name, executor);
    this._steps = steps;
  }
  ...
  generateContents(): JobContentsShape {
    ...
    return { steps: generatedSteps, ...super.generateContents() };
  }
  /**
   * Generate Job schema
  ...
  get steps(): AnyCommand[] | undefined {
    return this._steps;
  }
}
